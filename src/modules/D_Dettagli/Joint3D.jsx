...
function buildJointScene(THREE, scene, cfg, profilo, traveL, wireframe) {
  ...
  // ── Materials ──
  const matSteel = new THREE.MeshPhongMaterial({ color: 0x3b82f6, specular: 0x88aaff, shininess: 90, wireframe })
  const matPlate = new THREE.MeshPhongMaterial({ color: 0x1d4ed8, specular: 0x6699ff, shininess: 100, wireframe })
  ...
  const matStiff = new THREE.MeshPhongMaterial({ color: 0x0ea5e9, specular: 0x7dd3fc, shininess: 80, wireframe })
  const matConc  = new THREE.MeshPhongMaterial({ color: 0x78716c, shininess: 10, wireframe })
  const matCol   = new THREE.MeshPhongMaterial({ color: 0x1e40af, specular: 0x6699ff, shininess: 70, wireframe })
  ...
  if (cfg.tipo === 'endplate') {
    ...
    // Column (vertical HEB/IPE, simplified box)
    const colW = 0.2, colH = 1.2, colT = 0.015
    add(box(colT, colH, colW), matCol, 0, 0, 0)            // web
    add(box(colT, colH, colT), matCol, 0, 0, colW / 2)     // flange right
    add(box(colT, colH, colT), matCol, 0, 0, -colW / 2)    // flange left
    ...
    ;(cfg.bolts || []).forEach(bl => {
      ...
      const shankL = 0.08
      // shank
      add(cyl(bd / 2, bd / 2, shankL), matBolt, pt / 2 + shankL / 2, by, bx, 0, 0, Math.PI / 2)
      // head
      add(cyl(bd * 0.9, bd * 0.9, bd * 0.6, 6), matBolt, pt / 2 + shankL + bd * 0.3, by, bx, 0, 0, Math.PI / 2)
      // nut
  ...
  if (cfg.tipo === 'beambeam') {
    ...
    // Beam B (right)
    addBeam(traveL * 0.45, pt / 2 + traveL * 0.45 / 2, 0, new THREE.MeshPhongMaterial({ color: 0x1e40af, specular: 0x88aaff, shininess: 90, wireframe }))

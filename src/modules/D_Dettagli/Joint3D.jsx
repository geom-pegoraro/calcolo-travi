...
function buildJointScene(THREE, scene, cfg, profilo, traveL, wireframe) {
  ...
  if (cfg.tipo === 'cleat') {
    ...
    // ch = altezza NETTA anima trave (confinata tra le due flange, non tutta la colonna)
    const ch  = h - 2 * tf - 0.004
    const ct  = (cfg.cleat?.t || 10) * MM
    const cFT = 0.015

    // Pilastro HE verticale: flangia destra a x=0
    const colW = 0.2, colH = 1.4
    add(box(cFT, colH, colW),  matCol, -cFT/2, 0, 0)
    add(box(cFT, colH, cFT),   matCol, -cFT/2, 0,  colW/2)
    add(box(cFT, colH, cFT),   matCol, -cFT/2, 0, -colW/2)

    // Trave a destra, faccia sinistra a x = ct + cw
    addBeamRight(traveL * 0.5, ct + cw)

    // Squadrette ad L — centrate in Y=0, alte ch (= solo anima trave)
    ;[-1, 1].forEach(sign => {
      const zBase = sign * tw / 2
      // Gamba VERTICALE (contro flangia pilastro)
      add(box(ct, ch, cw), matPlate, ct/2, 0, zBase + sign * cw/2)
      // Gamba ORIZZONTALE (contro anima trave)
      add(box(cw, ch, ct), matPlate, ct + cw/2, 0, zBase + sign * ct/2)
    })

    // Bulloni sulla gamba verticale
    ;(cfg.bolts || []).forEach(bl => {
      const by = bl.y * MM
      const bd = (cfg.boltDiam || 20) * MM
      ...
      const bxC = ct / 2
      ;[-1, 1].forEach(sign => {
        const bz = sign * (tw/2 + cw * 0.35)
        add(cyl(bd/2, bd/2, sL),         matBolt, bxC, by, bz, 0, 0, Math.PI/2)
        add(cyl(bd*.9, bd*.9, bd*.6, 6), matBolt, bxC + sL/2 + bd*.3, by, bz, 0, 0, Math.PI/2)
        add(cyl(bd*.9, bd*.9, bd*.6, 6), matBolt, bxC - sL/2 - bd*.3, by, bz, 0, 0, Math.PI/2)
      })
    })

    // Saldatura gamba orizzontale → anima trave
    const wT = (cfg.welds?.[0]?.a || 6) * MM
    ;[-1, 1].forEach(sign => {

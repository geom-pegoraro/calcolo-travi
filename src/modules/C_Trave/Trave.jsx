...
import { Plus, Trash2, Calculator, Cpu } from 'lucide-react'
import { calcolaTrave, verificaSezione, verificaLegno, PROFILI, getProfilo, isLegno } from './TraveCalc'
...
export default function Trave({ onDataChange }) {
  ...
  const [schemaCfg, setSchemaCfg] = useState(DEFAULT_SCHEMA_CFG)
  ...
  function addCarico() {
    setCarichi(c => [...c, {
      id: Date.now(), tipo: 'puntuale', valore: 10,
      posizione: L / 2, lunghezza: null, direzioneUp: false,
    }])
  }
  function removeCarico(id)         { setCarichi(c => c.filter(x => x.id !== id)) }
  function updateCarico(id, key, v) { setCarichi(c => c.map(x => x.id === id ? { ...x, [key]: v } : x)) }

  const calcola = useCallback(() => {
    const p   = getProfilo(nomeP)
    const Iy  = p?.Iy || 5790
    const Ev  = E || 210000
    const res = calcolaTrave({ L: +L, E: Ev, Iy, carichi })
    const ver = legno
      ? verificaLegno(res.Mmax, p, tipoP, kmod)
      : verificaSezione(res.Mmax, p, +fyk)
    setResult(res)
    setVerifica(ver)
    onDataChange?.({ L: +L, profilo: nomeP, profiloData: p, carichi, result: res, verifica: ver })
  }, [L, nomeP, E, carichi, fyk, kmod, legno, tipoP, onDataChange])
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '340px 1fr', gap: 20, alignItems: 'start' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
          ...
          <div className="eng-card" style={{ padding: 16 }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
              ...
              <div>
                ...
                <select className="eng-select" value={tipoP} onChange={e => handleTipoChange(e.target.value)}>
                  <optgroup label="── Acciaio ──">
                    {['IPE','HEA','HEB'].map(k => <option key={k} value={k}>{k}</option>)}
                  </optgroup>
                  <optgroup label="── Legno Lamellare ──">
                    {['GL24h','GL28h','GL32h'].map(k => <option key={k} value={k}>{k}</option>)}
                  </optgroup>
                  <optgroup label="── Legno Massiccio ──">
                    {['C24','C16'].map(k => <option key={k} value={k}>{k}</option>)}
                  </optgroup>
                </select>
              </div>
              <div>
            ...
            {profilo && (
              <div style={{ marginTop: 12, padding: '8px 10px', background: 'var(--bg-secondary)',
                borderRadius: 6, display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 6 }}>
                {[
                  ['h', profilo.h, 'mm'], ['b', profilo.b, 'mm'],
                  ['Iy', profilo.Iy, 'cm⁴'], ['Wy', profilo.Wy, 'cm³'],
                  ['A', profilo.A, 'cm²'], ['E', profilo.E, 'MPa'],
                ].map(([k, v, u]) => (
          ...
          <div className="eng-card" style={{ padding: 16 }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
              <div>
                ...
                <input className="eng-input" type="number" value={E}
                  onChange={e => setE(+e.target.value)}/>
              </div>
              {legno ? (
                <div>
                  <label className="eng-label">kmod</label>
                  <select className="eng-select" value={kmod} onChange={e => setKmod(+e.target.value)}>
                    <option value={0.6}>0.60 — Perm./Cl.1</option>
                    <option value={0.7}>0.70 — Lungo/Cl.1</option>
                    <option value={0.8}>0.80 — Medio/Cl.1</option>
                    <option value={0.9}>0.90 — Corto/Cl.1</option>
                    <option value={1.1}>1.10 — Istant./Cl.1</option>
                  </select>
                </div>
              ) : (
                <div>
                  ...
                  <div style={{ display: 'flex', gap: 3 }}>
                    {[false, true].map(up => (
                      <button key={String(up)}
                        onClick={() => updateCarico(c.id, 'direzioneUp', up)}
                        title={up ? 'Freccia verso su' : 'Freccia verso giù'}
                        style={{
                          padding: '3px 8px', borderRadius: 4, cursor: 'pointer', fontSize: '0.85rem',
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
          ...
          <div>
            ...
            <TraveSchema2D
              L={L} carichi={carichi} RA={result?.RA} RB={result?.RB}
              schemaConfig={schemaCfg}
              onSchemaConfigChange={setSchemaCfg}
            />
          </div>
                ...
                <div className="eng-card" style={{ padding: '12px 16px', display: 'flex', alignItems: 'center', gap: 16 }}>
                  ...
                  <div style={{ flex: 1 }}>
                    <div style={{ fontFamily: 'Rajdhani', fontWeight: 700, fontSize: '0.9rem',
                      ...
                      {verifica.tipo === 'legno'
                        ? `VERIFICA FLESSIONE EC5 — ${verifica.ok ? 'SODDISFATTA' : 'NON SODDISFATTA'}`
                    ...
                    <div style={{ fontFamily: 'JetBrains Mono', fontSize: '0.75rem', color: 'var(--text-secondary)', marginTop: 2 }}>
                      ...
                      {verifica.tipo === 'legno' && (
                        <span style={{ color: 'var(--text-muted)', marginLeft: 8 }}>
                          (fm,k={verifica.fm_k} MPa · kmod={verifica.kmod} · γM={verifica.gammaM})
                        </span>
                      )}
                    </div>
                  </div>

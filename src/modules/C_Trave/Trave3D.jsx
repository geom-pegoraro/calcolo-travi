...
export default function Trave3D({ L = 6, profilo = null, carichi = [] }) {
  ...
  useEffect(() => {
    ...
    async function init() {
      ...
      const controls = createOrbitControls(THREE, camera, renderer.domElement, L, isFullscreenRef)
    ...
    <div style={{
      ...
      <div style={{
        ...
        <span style={{
          ...
          color: isFullscreen ? 'rgba(148,163,184,0.5)' : 'rgba(148,163,184,0.2)', letterSpacing: '0.08em',
        }}>
        ...
        <div style={{ display: 'flex', gap: 4 }}>
          ...
          <button onClick={() => setFullscreen(f => !f)} title={isFullscreen ? 'Esci da fullscreen' : 'Espandi per interagire'}
            style={{
              background: isFullscreen ? 'transparent' : 'rgba(59,130,246,0.12)',
              border: `1px solid ${isFullscreen ? 'var(--border)' : 'rgba(59,130,246,0.4)'}`,
              borderRadius: 4, padding: '3px 6px', cursor: 'pointer',
              color: isFullscreen ? 'var(--text-muted)' : 'var(--accent-blue)', display: 'flex', alignItems: 'center',
            }}>
...
function createOrbitControls(THREE, camera, domElement, L, isFullscreenRef) {
  ...
  window.addEventListener('mousemove', e => {
    if (!active()) { state.isRotating = false; state.isPanning = false; return }
  ...
  // Scroll zoom — solo in fullscreen

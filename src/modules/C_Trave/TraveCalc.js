...
export function calcolaTrave({ L, E, Iy, carichi = [], N = 200 }) {
  ...
  // ── 1a integrazione: θ_raw(x) = ∫₀ˣ M(t)dt  [kN·m²] (senza costante)
  const theta_raw = new Array(N + 1).fill(0)
  for (let i = 1; i <= N; i++) {
    theta_raw[i] = theta_raw[i - 1] + (M[i - 1] + M[i]) / 2 * (dx / 1000)
  }

  // C1 da condizione δ(L)=0:  ∫₀ᴸ (θ_raw + C1) dx = 0
  // → C1 = -(1/L) ∫₀ᴸ θ_raw dx
  let int_theta = 0
  for (let i = 1; i <= N; i++) {
    int_theta += (theta_raw[i - 1] + theta_raw[i]) / 2 * (dx / 1000)
  }
  const C1 = -int_theta / L   // costante di integrazione [kN·m]

  // ── 2a integrazione: δ_raw(x) = ∫₀ˣ (θ_raw(t) + C1) dt
  const delta_raw = new Array(N + 1).fill(0)
  for (let i = 1; i <= N; i++) {
    const th0 = theta_raw[i - 1] + C1
    const th1 = theta_raw[i]     + C1
    delta_raw[i] = delta_raw[i - 1] + (th0 + th1) / 2 * (dx / 1000)
  }

  // Dividi per EI, converti m→mm; freccia verso il basso = negativa per convenzione
  for (let i = 0; i <= N; i++) {
    d[i] = -(delta_raw[i] / EIkNm2) * 1000
  }

...
export default function TraveSchema2D({ L, carichi = [], RA, RB, schemaConfig, onSchemaConfigChange }) {
  ...
  // Default config — reazioni verso l'alto (fisicamente corrette per trave appoggiata)
  ...
  // ── Extra support (column symbol) ──
  const ExtraSupport = ({ x, y, up, color }) => (
    <g>
      <line x1={x} y1={y} x2={x} y2={up ? y - 28 : y + 28}
        stroke={color} strokeWidth={3}/>
      <line x1={x - 10} y1={up ? y - 28 : y + 28}
            x2={x + 10} y2={up ? y - 28 : y + 28}
    ...
    <div style={{ position: 'relative' }}>
      ...
      <svg width="100%" viewBox={`0 0 ${W} ${H}`}
        ...
        {Array.from({ length: Math.floor(L) + 1 }, (_, i) => i).map(i => (
          <line key={i} x1={toX(i)} y1={padY - 10} x2={toX(i)} y2={H - padY + 8}
        ...
        {/* ── Reazioni — sempre sotto il vincolo, direzione configurabile ── */}
        {RA != null && (() => {
          // supportY = base del simbolo vincolo (cerniera ~26px, carrello ~30px sotto beamY+10)
          const supportBase = beamY + 10 + 30
          const arrowY = cfg.reactionAUp
            ? supportBase + 35   // freccia punta su: la coda è in basso
            : supportBase + 5    // freccia punta giù: la coda è in alto
          return (
            <Arrow
              x={padX}
              y={arrowY}
        ...
        {/* ── Quote ── */}
        <line x1={padX} y1={H - 7} x2={W - padX} y2={H - 7} stroke={TEXT_C} strokeWidth={1}/>
        <line x1={padX} y1={H - 12} x2={padX} y2={H - 2} stroke={TEXT_C} strokeWidth={1}/>
        <line x1={W - padX} y1={H - 12} x2={W - padX} y2={H - 2} stroke={TEXT_C} strokeWidth={1}/>
        <text x={W / 2} y={H - 1} textAnchor="middle" fontSize={8.5} fill={TEXT_C} fontFamily="JetBrains Mono">
          L = {L} m
        </text>
